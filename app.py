...
def get_kpi_data():
    ...
    try:
        ...
        return jsonify({'total_students': total_students, 'total_dropout': total_dropout, 'dropout_rate': dropout_rate})
    except Exception as e:
        print(f"🔴 KPI Error: {e}")
...
def get_gemini_interventions(student_data, prediction_result):
    ...
    try:
        prompt = f"You are an expert educational counselor... (prompt text is the same)"
...
if __name__ == '__main__':
    ...
    # Render uses the 'gunicorn app:app' command from the dashboard settings.
